...
ARTIFACT_PATH = (
    ...
    / "technical_risk_ai_exposure_batch01_semiconductor_a_v1.json"
)
PHASE1_MAPPING_PATH = (
    PROJECT_ROOT
    / "data"
...
class TechnicalRiskAIExposureBatch01SemiconductorATestCase(unittest.TestCase):
    def setUp(self):
    ...
    def test_artifact_contains_exact_batch_membership(self):
        self.assertEqual(self.artifact["batch_id"], "SEMICONDUCTOR_A_BATCH_01")
        self.assertEqual(tuple(record["symbol"] for record in self.records), EXPECTED_SYMBOLS)
        self.assertEqual(len(self.records), 16)
        self.assertEqual(len({record["symbol"] for record in self.records}), 16)

    def test_batch_symbols_are_in_committed_current_semiconductor_universe(self):
        ...
        for record in self.records:
            ...
            self.assertEqual(phase1_record["broad_industry"], "半導體業")
            self.assertTrue(phase1_record["technology_related_preview"])
            self.assertEqual(record["current_official_broad_industry"], "半導體業")
            self.assertEqual(record["broad_industry"], "半導體業")
    ...
    def test_evidence_dates_are_pre_2024_and_lineage_is_present(self):
        ...
        for record in self.records:
            ...
            self.assertTrue(record["evidence_source_reference"].startswith("https://"))
    ...
    def test_categories_are_allowed_and_counts_are_fixed(self):
        ...
        self.assertEqual(
            self.artifact["classification_counts"],
            {
                "AI_ADJACENT": 3,
                "AI_HIGH": 5,
                "NON_AI_TECH_CONTROL": 1,
                "UNKNOWN": 7,
            },
        )

    def test_unknown_and_non_ai_control_audit_statuses_are_explicit(self):
        for record in self.records:
            ...
            if record["ai_exposure_category"] == TechnicalRiskAIExposureCategory.NON_AI_TECH_CONTROL.value:
                self.assertEqual(record["source_audit_status"], "FOUND_ELIGIBLE_PRIMARY_EVIDENCE")
                self.assertNotIn("keyword search", record["classification_reason"].lower())
    ...
    def test_synthetic_cutoff_publication_dates_are_rejected_by_audit(self):
        for record in self.records:
            ...
            if record["source_audit_status"] == "PUBLICATION_DATE_UNVERIFIED":
    ...
    def test_artifact_checksum_and_id_are_deterministic(self):
        ...
        self.assertEqual(
            ...
            f"technical_risk_ai_exposure_batch01_semiconductor_a_{checksum[:16]}",
        )
