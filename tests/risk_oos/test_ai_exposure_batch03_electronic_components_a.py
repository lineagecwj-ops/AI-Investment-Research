...
ARTIFACT_PATH = (
    ...
    / "technical_risk_ai_exposure_batch03_electronic_components_a_v1.json"
)
PHASE1_MAPPING_PATH = (
    PROJECT_ROOT
    / "data"
...
class TechnicalRiskAIExposureBatch03ElectronicComponentsATestCase(unittest.TestCase):
    def setUp(self):
    ...
    def test_artifact_contains_exact_batch_membership(self):
        self.assertEqual(self.artifact["batch_id"], "ELECTRONIC_COMPONENTS_A_BATCH_03")
        self.assertEqual(tuple(record["symbol"] for record in self.records), EXPECTED_SYMBOLS)
        self.assertEqual(len(self.records), 17)
        self.assertEqual(len({record["symbol"] for record in self.records}), 17)

    def test_batch_symbols_are_in_committed_current_electronic_components_universe(self):
        ...
        for record in self.records:
            ...
            self.assertEqual(phase1_record["broad_industry"], "電子零組件業")
            self.assertTrue(phase1_record["technology_related_preview"])
            self.assertEqual(record["current_official_broad_industry"], "電子零組件業")
            self.assertEqual(record["broad_industry"], "電子零組件業")
    ...
    def test_artifact_checksum_and_id_are_deterministic(self):
        ...
        self.assertEqual(
            ...
            f"technical_risk_ai_exposure_batch03_electronic_components_a_{checksum[:16]}",
        )
