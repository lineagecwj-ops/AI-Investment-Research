...
class DashboardFormattingTestCase(unittest.TestCase):
    ...
    def test_swing_research_tab_uses_explicit_session_result(self):
        ...
        self.assertIn('"Swing Research（波段研究）"', app_source)
        self.assertIn('"歷史案例"', app_source)
        ...
        self.assertIn('"執行歷史回放"', app_source)
        self.assertIn('"執行多日期歷史回放"', app_source)
        self.assertIn('if submitted:', app_source)
        ...
        self.assertIn("股票來源", app_source)
        helper_source = (SRC_PATH / "swing_research_dashboard.py").read_text(encoding="utf-8")
        self.assertIn('HISTORICAL_REPLAY_MODE = "Historical Replay"', helper_source)
        self.assertIn('WALK_FORWARD_REPLAY_MODE = "Walk-Forward Replay"', helper_source)
        self.assertIn("FROZEN_TWSE_RESEARCH_SOURCE", app_source)
        self.assertIn("研究股票池目前無法載入。", app_source)
        self.assertIn("load_frozen_twse_research_source()", app_source)
        self.assertNotIn("[\"1101.TW\"", app_source)
        self.assertIn("指定回放日期", app_source)
...
"""
    ...
    def test_universe_management_tab_exists(self):
        ...
        self.assertIn('"研究股票池"', app_source)
    ...
    def test_swing_research_wording_preserves_research_semantics(self):
        ...
        self.assertIn("歷史命中率", swing_page_source)
        self.assertIn("回放當時可知歷史命中率", swing_page_source)
        self.assertIn("回放日期後的實際歷史結果", swing_page_source)
        self.assertIn("回放期數", helper_source)
        self.assertIn("候選出現次數", helper_source)
        self.assertIn("回放結果案例圖", swing_page_source)
        self.assertIn("signal date 之後資料只用於歷史事後驗證", swing_page_source)
        self.assertIn("已解析歷史樣本數", swing_page_source)
        self.assertIn("研究優先順序", swing_page_source)
        ...
        self.assertIn("不代表未來上漲機率", combined_source)
    ...
    def test_oos_validation_dashboard_ui_exists_and_preserves_semantics(self):
        ...
        self.assertIn("樣本外驗證", oos_page_source)
        self.assertIn("開發期間", oos_page_source)
        self.assertIn("驗證期間", oos_page_source)
        self.assertIn("保留樣本期間", oos_page_source)
        self.assertIn("歷史命中率", oos_page_source)
        self.assertIn("已解析樣本", oos_page_source)
        self.assertIn("候選出現期間比例", oos_page_source)
        self.assertIn("研究規格識別碼", oos_page_source)
